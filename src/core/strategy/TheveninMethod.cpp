...
enum State { ConstantCurrentBalancing,
             ConstantCurrent,
             RthMesurment,
             LastRthMesurment,
             LastConstantCurrent,
             ConstantVoltageBalancing };
...
AnalogInputs::ValueType TheveninMethod::getReadableWiresRth() {
    int32_t vo   = AnalogInputs::getRealValue( AnalogInputs::Vout );
    int16_t vbat = AnalogInputs::getRealValue( AnalogInputs::VoutBalancer );
    uint16_t i   = AnalogInputs::getRealValue( AnalogInputs::Iout );
    int32_t rth  = Resistance::CalculateResistance( i, vo - vbat );
    return Resistance::getReadableRth( rth );
}
...
AnalogInputs::ValueType TheveninMethod::calculateNewI( bool isEndVout, AnalogInputs::ValueType I ) {
    ...
    bool outputStable = AnalogInputs::isOutStable();
    bool updateI      = outputStable || ( isEndVout && newI_ != 0 );
    ...
    if ( updateI ) {
        ...
        // low pass filter
        newI_ = ( (uint32_t)newI_ + I ) / 2;
        // apply ramp
        int16_t step = ClipL( ( (int32_t)newI_ - I ), -CURRENT_RAMP, CURRENT_RAMP );
        newI_        = I + step;
        // normalize
    ...
    // whenever newI_ is set to zero we also need to reset resistances calculations so they can be calculated again
    if ( newI_ == 0 ) {
        tVout_.Rth.Reset();
        for ( uint8_t c = 0; c < MAX_BALANCE_CELLS; c++ ) {
            tBal_[c].Rth.Reset();
...
void TheveninMethod::measureRth( AnalogInputs::ValueType I ) {
    tVout_.Rth.AddPoint( I, AnalogInputs::getVbattery() );

    for ( uint8_t c = 0; c < MAX_BALANCE_CELLS; c++ ) {
        if ( AnalogInputs::connectedBalancePortCells & ( 1 << c ) )
            tBal_[c].Rth.AddPoint( I, Balancer::getPresumedV( c ) );
    }
}
...
void TheveninMethod::displayDebugPage( int pindex ) {
    switch ( pindex ) {
    case 0:
        lcdSetCursor0_0();
        lcdPrint_P( "pa " );
        lcdPrintUnsigned( Resistance::points_added, 6 );
        break;

    case 1:
        lcdSetCursor0_0();
        lcdPrintUnsigned( tVout_.Rth.i_initial, 4 );
        lcdPrintSpace1();
        lcdPrintUnsigned( tVout_.Rth.i_last, 4 );

        lcdSetCursor0_1();
        lcdPrintUnsigned( tVout_.Rth.v_initial, 4 );
        lcdPrintSpace1();
        lcdPrintUnsigned( tVout_.Rth.v_last, 4 );
        break;
    case 2:
    case 3:
    case 4:
    case 5: {
        ...
        int balindex = pindex - 2;
        lcdSetCursor0_0();
        lcdPrintUnsigned( tBal_[balindex].Rth.i_initial, 4 );
        lcdPrintSpace1();
        lcdPrintUnsigned( tBal_[balindex].Rth.i_last, 4 );

        lcdSetCursor0_1();
        lcdPrintUnsigned( tBal_[balindex].Rth.v_initial, 4 );
        lcdPrintSpace1();
        lcdPrintUnsigned( tBal_[balindex].Rth.v_last, 4 );
    } break;
    default:
        break;
    }
